...
import Item, { ItemType, MovementType } from './item';
...
class Board {
  ...
  updateBoardWithMatrix = (matrix: ItemType[][]) => {
    ...
    matrix.forEach((row: ItemType[], rowIndex: number) => {
      row.forEach((item: ItemType, colIndex: number) => {
        newBoard[`${rowIndex}|${colIndex}`] = { item: new Item(item) };
      });
    });
